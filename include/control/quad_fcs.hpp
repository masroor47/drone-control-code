...
#include "control/quad_pid_config.hpp"
...
public:
    ...
    explicit quad_fcs(const config& cfg)
        : config_(cfg),
          imu_(nullptr),
          barometer_(nullptr),
          motors_(),
          pid_configs_(quad_pid_config::default_pid_configs()) {
...
private:
    ...
    std::array<std::unique_ptr<esc_controller>, 4> motors_;
